...
const EmployeeCard = ({ employee, onDelete, onEdit }) => {
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            // Re-fetch parent component dataset instantly 
            onDelete()
        } catch (error) {
            console.error("Delete call request failed:", error)
            alert(error.response?.data?.message || "Failed to remove employee record.")
        }
    }
  ...
  return (
    <div className="group relative card card-hover overflow-hidden">
        ...
        {!employee.isDeleted && (
            <div className="absolute inset-0 bg-gradient-to-t from-indigo-700/20 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end justify-center gap-3 pb-6">
